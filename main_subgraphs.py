...
#-----------------------------------------------------
    ...
    for n in range(num_subgraphs):
        ...
        input_path = args.input_path[:-1] + str(n+1) + "/"

        with open(input_path+args.graph_file, 'rb') as f:
            low_high_graph = pickle.load(f)

        with open(input_path+args.target_file, 'rb') as f:
        ...
        if args.loss_fn == 'weighted_mse_loss' or args.loss_fn == "quantized_loss" or args.loss_fn == "quantized_focal_loss":
            with open(input_path+args.weights_file, 'rb') as f:
        ...
        datasets_train.append(dataset_graph_train), datasets_val.append(dataset_graph_val)

    dataset_graph_train = torch.utils.data.ConcatDataset(datasets_train)
    dataset_graph_val = torch.utils.data.ConcatDataset(datasets_val)
