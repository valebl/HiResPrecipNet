...
class Dataset_Graph(Dataset):
    ...
    def _get_target(self, time_index: int):
        if self.targets is None:
            return None
        elif self.targets[:,time_index].dtype == torch.int:
            return torch.LongTensor(self.targets[:,time_index])
        elif self.targets[:,time_index].dtype == torch.float:
    ...
    def _get_additional_feature(self, time_index: int, feature_key: str):
        ...
        if feature.dtype == torch.int:
            return torch.LongTensor(feature)
        elif feature.dtype == torch.float:
            return torch.FloatTensor(feature)
        elif feature.dtype == torch.bool:
    ...
    def __getitem__(self, time_index: int):
        ...
        snapshot = HeteroData(y=y, **additional_features)
        
        snapshot.train_mask = train_mask
