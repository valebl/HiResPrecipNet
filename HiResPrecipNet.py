...
from torch_geometric.nn import GATv2Conv, HeteroConv, GraphConv, GCNConv
...
class HiResPrecipNet_RNN(nn.Module):
    ...
    def __init__(self, encoding_dim=128, seq_l=25, h_in=5*5, h_hid=5*5, n_layers=2, high_in=1, low2high_out=64, high_out=64):
        super(HiResPrecipNet_RNN, self).__init__()
    ...
    def forward(self, data):    
        ...
        encod_rnn = encod_rnn.flatten(start_dim=1)
        encod_rnn = self.dense(encod_rnn)
        encod_low2high  = self.downscaler((encod_rnn, data['high'].x), data.edge_index_dict[('low','to','high')])
        encod_low2high  = torch.concatenate((data['high'].z_std, encod_low2high ),dim=-1)
        encod_high = self.processor(encod_low2high , data.edge_index_dict[('high','within','high')])
        y_pred = self.predictor(encod_high)
...
class HiResPrecipNet_encod(nn.Module):
    ...
    def __init__(self, low_in=5*5*5, high_in=1, low2high_out=64, high_out=64):
        super(HiResPrecipNet_encod, self).__init__()
    ...
    def forward(self, data):        
        ...
        encod_high = self.processor(encod_low2high , data.edge_index_dict[('high','within','high')])
        y_pred = self.predictor(encod_high)
        return y_pred, encod_low2high

class HiResPrecipNet_0502(nn.Module):
    
    def __init__(self, low_in=25*5*5, high_in=1, low2high_out=32, high_out=32):
        super(HiResPrecipNet_0502, self).__init__()

        self.downscaler = GATv2Conv((low_in, high_in), out_channels=low2high_out, dropout=0.4, heads=4, aggr='mean', add_self_loops=True, bias=True)
        
        self.processor = geometric_nn.Sequential('x, edge_index', [
            (geometric_nn.BatchNorm(low2high_out*4), 'x -> x'),
            (GATv2Conv(in_channels=low2high_out*4, out_channels=high_out, heads=4, dropout=0.4, aggr='mean', add_self_loops=True, bias=True, concat=False), 'x, edge_index -> x'),
            (geometric_nn.BatchNorm(high_out), 'x -> x'), 
            (GATv2Conv(in_channels=high_out, out_channels=high_out, heads=4, dropout=0.4, aggr='mean', add_self_loops=True, bias=True, concat=False),'x, edge_index -> x'),
            (geometric_nn.BatchNorm(high_out), 'x -> x'),
            (GATv2Conv(in_channels=high_out, out_channels=high_out, heads=4, dropout=0.4, aggr='mean', add_self_loops=True, bias=True, concat=False),'x, edge_index -> x'),
            (geometric_nn.BatchNorm(high_out), 'x -> x'),
            (GATv2Conv(in_channels=high_out, out_channels=high_out, heads=4, dropout=0.4, aggr='mean', add_self_loops=True, bias=True, concat=False),'x, edge_index -> x'),
...
class HiResPrecipNet_GCN(nn.Module):
    ...
    def __init__(self, low_in=25*5*5, high_in=1, low2high_out=256, high_out=128):
        super(HiResPrecipNet_GCN, self).__init__()
        ...
        self.processor = geometric_nn.Sequential('x, edge_index', [
            ...
            (geometric_nn.BatchNorm(high_out), 'x -> x'),
            nn.ReLU(),
            (GCNConv(high_out, 1), 'x, edge_index -> x'),
            nn.ReLU(),
            ])
...
class HiResPrecipNet_GCN_2(nn.Module):
    
    def __init__(self, low_in=25*5*5, high_in=1, low2high_out=64, high_out=32):
        super(HiResPrecipNet_GCN_2, self).__init__()
...
class HiResPrecipNet_GCN_3(nn.Module):
    
    def __init__(self, low_in=25*5*5, high_in=1, low2high_out=64, high_out=32):
        super(HiResPrecipNet_GCN_3, self).__init__()
